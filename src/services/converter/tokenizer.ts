...
function tokenize(input: string): Token[] {
	...
	function readString(quote: string): Token {
			...
			if (input[pos] === '\\') {
				...
				if (pos >= input.length) {
					throw new TokenizerError(
						'Cadena sin terminar: secuencia de escape al final',
						start,
						input
					)
				}
		...
		if (pos >= input.length) {
			throw new TokenizerError(
				`Cadena sin terminar: falta la comilla de cierre ${quote}`,
				start,
				input
			)
		}
	...
	function readWord(): { word: string; start: number; length: number } {
		const start = pos
		let word = ''
		...
		if (/[a-zA-Z_]/.test(ch)) {
			...
			if (upper === 'AND' || upper === 'OR') {
				...
				if (pos < input.length && input[pos] !== "'" && input[pos] !== '"') {
					...
					if (pos > rxStart) {
						tokens.push(
							createToken(TokenType.STRING, input.substring(rxStart, pos), rxStart, pos - rxStart)
						)
					}
				}
...
export type { Token, TokenTypeValue }
